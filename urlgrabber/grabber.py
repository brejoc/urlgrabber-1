...
class URLGrabber(object):
    ...
    def _retry(self, opts, func, *args):
        ...
        while 1:
            ...
            callback  = None
            if DEBUG:
                DEBUG.info('attempt %i/%s: %s', tries, opts.retry, args[0])
            try:
                r = func(*(opts,) + args, **{})
                if DEBUG:
                    DEBUG.info('success')
            ...
            if (retrycode is not None) and (retrycode not in opts.retrycodes):
                message = 'retrycode (%i) not in list %s, re-raising' % (retrycode, opts.retrycodes)
                if DEBUG:
                    DEBUG.info(message)
                raise sys.exc_info()[1]
    ...
    def urlgrab(self, url, filename=None, opts=None, **kwargs):
        ...
        opts = (opts or self.opts).derive(**kwargs)
        if DEBUG:
            DEBUG.debug('combined options: %s' % repr(opts))
        (url, parts) = opts.urlparser.parse(url, opts)
        (scheme, host, path, parm, query, frag) = parts
...
class PyCurlFileObject(object):
    def __init__(self, url, filename, opts):
        ...
        self.url = url
        self.scheme = urlsplit(self.url)[0]
    ...
    def _hdr_retrieve(self, buf):
        ...
        try:
            ...
            if buf.lower().find('location') != -1:
                ...
                self.scheme = urlsplit(location)[0]
                self.url = location
    ...
    def _return_hdr_obj(self):
        ...
        hdrfp.seek(0)
        # mimetools is only availalbe in Python2
        if 'mimetools' in sys.modules:
    ...
    def _build_range(self):
        reget_length = 0
        rt = None
        if self.opts.reget and isinstance(self.filename, six.string_types):
    ...
    def _do_grab(self):
        ...
        if self._complete:
            return
        _was_filename = False
        if isinstance(self.filename, six.string_types) and self.filename:
            _was_filename = True
...
class _ExternalDownloaderPool:
    ...
    def perform(self):
        ...
        for fd, event in self.epoll.poll():
            ...
            host = urlsplit(done[0][0].url).netloc
    ...
    def abort(self):
        for dl in list(self.running.values()):
            self.epoll.unregister(dl.stdout)
            dl.abort()
        for dl in list(self.cache.values()):
            dl.abort()
...
class _TH:
    ...
    def update(url, dl_size, dl_time, ug_err, baseurl=None):
        ...
        host = urlsplit(url).netloc.split('@')[-1] or baseurl
        if not host: return
    ...
    def estimate(baseurl):
        ...
        host = urlsplit(baseurl).netloc.split('@')[-1] or baseurl
